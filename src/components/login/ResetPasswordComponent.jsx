...
import { executeResetPassword } from "../../api/LoginApiService";
...
function ResetPasswordComponent() {
  ...
  const passwordToken = new URLSearchParams(location.search).get(
    "passwordToken"
  );
  ...
  const updateAllFieldsValid = () => {
    setAllFieldsValid(
      validPassword && confirmPassword === password && password !== ""
    );
  };
    ...
    <div className="flex flex-row items-start justify-center h-screen bg-gray-800">
      <div className="bg-gray-900 shadow-md rounded-l-xl px-8 pt-6 pb-8 my-8 w-full min-h-96 max-w-md">
        ...
        <div className="flex items-start justify-evenly mt-4">
          <Link
            to="/login"
            className="text-blue-600 ease-in duration-300 hover:text-blue-300 text-sm"
          >
            Back to Login
          </Link>
        </div>
